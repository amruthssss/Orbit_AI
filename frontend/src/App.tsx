...
function KnowledgeView() {
  ...
  return <div className="view"><ViewHeading title="Knowledge base" subtitle="Ground answers in your own documents with lightweight local retrieval." action={<label className={`button primary ${uploading ? "is-loading" : ""}`}>＋ {uploading ? "Uploading…" : "Add document"}<input type="file" hidden accept=".pdf,.txt,.md,.json,.csv,application/pdf,text/plain,text/markdown,application/json,text/csv" onChange={(event) => event.target.files?.[0] && void upload(event.target.files[0])} /></label>} /><Card><div className="search-row"><div className="input-with-icon">⌕<input aria-label="Search indexed knowledge" value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => event.key === "Enter" && void search()} placeholder="Search indexed knowledge…" /></div><button className="button" onClick={() => void search()} disabled={!query.trim() || state === "loading"}>Search</button></div><StatusMessage state={state === "error" ? "error" : results.length ? "success" : "idle"} error={error} success={results.length ? `Retrieved ${results.length} matching source${results.length === 1 ? "" : "s"}.` : undefined} />{results.length > 0 && <div className="result-list"><div className="result-heading">Retrieved context <span>{results.length} matches</span></div>{results.map((result) => <div className="result" key={result.name + result.text}><div><strong>{result.name}</strong><small>match {Math.round(result.score * 100)}%</small></div><p>{result.text}</p><span className="source-chip">Source · indexed document</span></div>)}</div>}</Card><div className="section-title">Indexed documents <span>{docs.length}</span></div><Card className="table-card">{state === "loading" && !docs.length ? <StatusMessage state="loading" /> : docs.length ? docs.map((doc) => <div className="table-row" key={doc.id}><span className="file-icon">▤</span><div><strong>{doc.name}</strong><small>{doc.collection} · {doc.chunks} chunks</small></div><span className="muted">{doc.characters.toLocaleString()} chars</span><button className="button subtle" onClick={() => void remove(doc.id)}>Delete</button></div>) : <Empty text="No documents yet. Upload a PDF or text file to create your first collection." />}</Card></div>;
}
